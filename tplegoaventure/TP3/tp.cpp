...
void Display_INTERSECTION_SphereCylinder(Vec3 cylAxisOrigin, Vec3 cylAxisVector, double cylRayon, Vec3 sphCentre, double sphRayon, double resolution) {
    double cylAxisLength = sqrt(pow(cylAxisVector[0], 2) + pow(cylAxisVector[1], 2) + pow(cylAxisVector[2], 2));
    Vec3 cylAxisDir = cylAxisVector / cylAxisLength;

    // Parcourir une grille de points à l'intérieur de la sphère
    for (double x = sphCentre[0] - sphRayon; x <= sphCentre[0] + sphRayon; x += resolution) {
        for (double y = sphCentre[1] - sphRayon; y <= sphCentre[1] + sphRayon; y += resolution) {
            for (double z = sphCentre[2] - sphRayon; z <= sphCentre[2] + sphRayon; z += resolution) {
                Vec3 point(x, y, z);

                // Vérifier si le point est à l'intérieur de la sphère
                double distanceToSphereCenter = sqrt(pow(point[0] - sphCentre[0], 2) +
                                                     pow(point[1] - sphCentre[1], 2) +
                                                     pow(point[2] - sphCentre[2], 2));
                if (distanceToSphereCenter <= sphRayon) {
                    // Vérifier si le point est à l'intérieur du cylindre
                    Vec3 pointToCylAxisOrigin = point - cylAxisOrigin;
                    double projectionLength = pointToCylAxisOrigin[0] * cylAxisDir[0] +
                                              pointToCylAxisOrigin[1] * cylAxisDir[1] +
                                              pointToCylAxisOrigin[2] * cylAxisDir[2];

                    Vec3 closestPointOnAxis = cylAxisOrigin + projectionLength * cylAxisDir;
                    double distanceToCylAxis = sqrt(pow(point[0] - closestPointOnAxis[0], 2) +
                                                    pow(point[1] - closestPointOnAxis[1], 2) +
                                                    pow(point[2] - closestPointOnAxis[2], 2));

                    if (distanceToCylAxis <= cylRayon && projectionLength >= 0 && projectionLength <= cylAxisLength) {
                        DisplayVoxel(point, resolution);
                    }
                }
            }
        }
...
void Display_SOUSTRACTION_SphereCylinder(Vec3 centreSphere, double rayonSphere, Vec3 axisOriginCylinder, Vec3 axisVectorCylinder, double rayonCylinder, double resolution) {
    double cylAxisLength = sqrt(pow(axisVectorCylinder[0], 2) + pow(axisVectorCylinder[1], 2) + pow(axisVectorCylinder[2], 2));
    Vec3 cylAxisDir = axisVectorCylinder / cylAxisLength;

    // Parcourir une grille de points à l'intérieur de la sphère
    for (double x = centreSphere[0] - rayonSphere; x <= centreSphere[0] + rayonSphere; x += resolution) {
        for (double y = centreSphere[1] - rayonSphere; y <= centreSphere[1] + rayonSphere; y += resolution) {
            for (double z = centreSphere[2] - rayonSphere; z <= centreSphere[2] + rayonSphere; z += resolution) {
                Vec3 point(x, y, z);

                // Vérifier si le point est à l'intérieur de la sphère
                double distanceToSphereCenter = sqrt(pow(point[0] - centreSphere[0], 2) +
                                                     pow(point[1] - centreSphere[1], 2) +
                                                     pow(point[2] - centreSphere[2], 2));
                if (distanceToSphereCenter <= rayonSphere) {
                    // Vérifier si le point est à l'intérieur du cylindre
                    Vec3 pointToCylAxisOrigin = point - axisOriginCylinder;
                    double projectionLength = pointToCylAxisOrigin[0] * cylAxisDir[0] +
                                              pointToCylAxisOrigin[1] * cylAxisDir[1] +
                                              pointToCylAxisOrigin[2] * cylAxisDir[2];

                    Vec3 closestPointOnAxis = axisOriginCylinder + projectionLength * cylAxisDir;
                    double distanceToCylAxis = sqrt(pow(point[0] - closestPointOnAxis[0], 2) +
                                                    pow(point[1] - closestPointOnAxis[1], 2) +
                                                    pow(point[2] - closestPointOnAxis[2], 2));

                    if (!(distanceToCylAxis <= rayonCylinder && projectionLength >= 0 && projectionLength <= cylAxisLength)) {
                        DisplayVoxel(point, resolution);
                    }
                }
            }
...
void Display_UNION_SphereCylinder(Vec3 centreSphere, double rayonSphere, Vec3 axisOriginCylinder, Vec3 axisVectorCylinder, double rayonCylinder, double resolution){
    DisplaySphereVolumic(centreSphere,rayonSphere,resolution);
    DisplayCylinderVolumic(axisOriginCylinder,axisVectorCylinder,rayonCylinder,resolution);
}
float generateRandomFloat() {
    // Moteur de génération de nombres aléatoires
    std::random_device rd;
    std::mt19937 gen(rd());

    // Distribution uniforme entre 0.0 et 1.0
    std::uniform_real_distribution<float> dis(0.4, 0.8);
    float x = dis(gen);
    return x;
}
void display () {
    ...
    //DisplaySphereVolumic(Vec3(0.1,0.1,0.5),0.4,0.02);
    //DisplayCylinderVolumic(Vec3(0,0,0.5),Vec3(0,0,1),0.4,0.02); // Attention, le fonction ne s'allonge que via l'axe z 
    //Display_INTERSECTION_SphereCylinder(Vec3(0,0,0.5),Vec3(0,0,1),0.4,Vec3(0,0,0.5),0.4,0.02);
    //Display_SOUSTRACTION_SphereCylinder(Vec3(0.1,0.1,0.5),0.4,Vec3(0,0,0.5),Vec3(0,0,1),0.4,0.02);
    Display_UNION_SphereCylinder(Vec3(0.1,0.1,0.5),0.4,Vec3(0,0,0.5),Vec3(0,0,1),0.4,0.02);
    glColor3f(0.8,1,0.5);
